...
import com.example.aleppocollage.ui.registerteacher.RegisterTeacherViewModel
...
class AbsenceStudentFragment : Fragment(R.layout.fragment_absence_student) {
    ...
    private val absenceStudentViewModel by viewModels<AbsenceStudentViewModel>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnAbsenceFragmentRecordTest.setOnClickListener {
            if (binding.txtAbsenceStudentFragmentChoseDate.text == resources.getString(R.string.enterDateToday)) {
                Toasty.warning(
    ...
    private fun getAbsenceStudent(studentID: Int, date: String) {
        absenceStudentViewModel.getAbsenceStudent(studentID, date)
        absenceStudentViewModel.absenceStudent?.observe(viewLifecycleOwner, Observer {
            when (it.status) {
                Status.LOADING -> {
                    Log.d("state", "LOADING")
